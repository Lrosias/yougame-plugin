...
This repo is the plugin that teaches your coding agent to do the part before the upload:
make the game static and self-contained, add the leaderboard, run the same pre-upload
checks the upload page runs, and hand you a zip that is ready to drop.
...
| Prompts | `make_yougame_ready`, `add_leaderboard`, `make_playable_on_phones`, `add_multiplayer`. |

Everything is a public read: no account, no token, no key. The upload itself stays yours —
it needs your account, and the zip is drag and drop at https://yougame.co/upload.
